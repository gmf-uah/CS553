...
export class DatabaseService {
	...
	public buildReplaceQuery(
		...
		const fieldsToParameterize = allWritableFields.filter((fieldName) =>
			providedFieldSet.has(fieldName),
		);
		// make the title = $(1), description = $(2), status = $(3) etc
		const assignmentParts = this.buildParameterizedAssignments(
			payload,
			fieldsToParameterize,
		);
		// map those parameters to their fields
		const parameterizedAssignments = new Map(
			assignmentParts.assignments.map((assignment, i) => [
				fieldsToParameterize[i],
				assignment,
			]),
		);
		...
		const assignments = allWritableFields
			.map(
				(fieldName) =>
					parameterizedAssignments.get(fieldName) ?? `${fieldName} = DEFAULT`,
			)
			.join(", ");
	...
	public async executeSingleRowQuery<T extends QueryResultRow>(
		query: SqlQuery,
	): Promise<T | null> {
